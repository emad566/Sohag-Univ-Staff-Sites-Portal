...
<div class="panel panel-default">
    <div class="panel-heading">{{ trans('main.manageResearchs') }}<a href="{{ url('stuff/researchs/create') }}" class="btn btn-primary newRecored pull-left"><i class="fas fa-calendar-plus"></i> {{ trans('main.addNew') }} </a></div>
    @include("partials.errors")
    @include("partials.success")
    <div class="panel-body panelBodyForm">

        @if($user->researchs()->count()<1)
                ...
                @if($user->researchs)
                    ...
                    @foreach($researchs as $research)
                        <tr>
                            <td><?php echo ++$i; ?></td>
                            <td>
                                <a href="{{ url('stuff/researchs') }}/{{ $research->id }}/edit"><i class="fas fa-edit delEdit"></i></a>
                            ...
                            <td><a href="{{ url('stuff/researchs') }}/{{ $research->id  }}">{{ $research->title }}</a></td>
                            <td>{{ $research->subject->title }}</td>
                            <td><a href="{{ url('stuff/researchs/'.$research->id.'?p=researchs') }}" target="_blank"> {{ $research->answers()->count() }}  <i class="fas fa-file-signature"></i> | {{ trans('main.gView') }}</a></td>
                        </tr>  
                    @endforeach
                @endif
            </tbody>
...
<script>
    $(document).ready(function(){
        $( "#subject" ).change(function() {
            window.location.replace('{{ url('/stuff/sub/researchs/') }}/' + $( "#subject" ).val() + '?p=researchs')
        });
    })
</script>
