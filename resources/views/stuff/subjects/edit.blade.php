...
<div class="panel panel-default">
    <div class="panel-heading">{{ trans('main.subjectEditHead') }}: {{ $subject->title }}  
    ...
    <div class="panel-body panelBodyForm">
        <form id='CreateUser' class='CreateUser form-horizontal panelForm' method='POST' action="{{ route('subjects.update', [$subject->id]) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input type='hidden' name='_method' value='PUT'>

            <div class="form-group{{ $errors->has('title') ? ' has-error' : '' }}">
                <label for="title"> *{{ trans('main.gTitle') }}</label>
                <input value="@if(old('title')){{ old('title') }}@else{{ $subject->title }}@endif" type="text" name="title" required class="form-control name" id="title" placeholder="{{ trans('main.gTitle') }}">
            ...
            <div class="form-group{{ $errors->has('content') ? ' has-error' : '' }}">
                ...
                <textarea required name="content" id="content" class="form-control content editor" placeholder="">@if(old('content')){{ old('content') }}@else{{ $subject->content }}@endif</textarea>
        ...
        <form id='file_id' class='file_id dropzone' method='POST' action='{{ route('files.store', [])}}' enctype="multipart/form-data">
            ...
            <input type='hidden' name='fileable_id' value='{{ $subject->id }}'>
